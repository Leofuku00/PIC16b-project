...
from clarity_models import ModelRegistry
from clarity_types import FoldEval, ModelSpec, RunConfig


def default_config() -> RunConfig:
    """Return the default runtime configuration for local experiments."""
    return RunConfig(input_csv="rmp_all_schools_reviews_small.csv", output_csv="classification_results.csv", output_cv_folds_csv="classification_results_cv_folds.csv", output_model_summary_csv="classification_results_model_summary.csv", k_values=[3, 5, 8, 10], lsvc_safe_k_values=[3, 5], test_size=0.2, cv_splits=5, lsvc_safe_cv_splits=2, random_state=42, max_rows=None, requested_models=None, include_test_row=False)
...
class CrossValidationRunner:
    ...
    def __init__(self, feature_pipeline: FeaturePipeline):
        """Bind the reusable feature pipeline for fold-level featurization."""
    ...
    def grouped_cv_search(self, df_train: pd.DataFrame, tag_cols: Sequence[str], k_values: Sequence[int], specs: Sequence[ModelSpec], cv_splits: int) -> pd.DataFrame:
        ...
        y_all = df_train["clarityRating"].to_numpy()
...
class ExperimentRunner:
    ...
    def __init__(self, config: RunConfig):
        ...
        self.config = config
        self.dataset_manager = ReviewDatasetManager()
        self.feature_pipeline = FeaturePipeline(random_state=config.random_state)
        self.cv_runner = CrossValidationRunner(feature_pipeline=self.feature_pipeline)
    ...
    def _evaluate_test(self, df_train: pd.DataFrame, df_test: pd.DataFrame, tag_cols: Sequence[str], best_k: int, best_spec: ModelSpec) -> dict:
        ...
        print(f"k={best_k}, model={best_spec.name}")
        print("\n[Test] Metrics")
    ...
    def run(self) -> None:
        ...
        df_cv_full = self._run_cv_block(df_train=df_train, tag_cols=tag_cols, specs=full_specs, k_values=self.config.k_values, cv_splits=self.config.cv_splits, label=f"full mode for models {[s.name for s in full_specs]}")
        if df_cv_full is not None:
            cv_parts.append(df_cv_full)

        df_cv_safe = self._run_cv_block(df_train=df_train, tag_cols=tag_cols, specs=safe_specs, k_values=self.config.lsvc_safe_k_values, cv_splits=self.config.lsvc_safe_cv_splits, label="linear_svc safe mode")
